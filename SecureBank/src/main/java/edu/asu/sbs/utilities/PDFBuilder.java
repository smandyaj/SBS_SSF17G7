...
public class PDFBuilder extends AbstractITextPdfView {
	...
	protected void buildPdfDocument(Map<String, Object> viewModel, Document doc,
		...
		if(user.getCustomerAddress() != null && !user.getCustomerAddress().isEmpty()) {
			addressLine2 = user.getCustomerAddress();
		}
		
		doc.add(new Paragraph(user.getCustomerAddress() + " " + addressLine2));
		...
		for (Transaction transaction : transactions) {			
			...
			table.addCell(transaction.getTransactionType()==0 ? transaction
